...
def _is_valid_channel(df: pd.DataFrame, support: Dict, resistance: Dict, params: dict) -> bool:
    ...
    return pct_in_channel >= params.get("min_pct_in_channel", 70.0)  # tunable (R-2), default 70%
...
def detect_consolidation_patterns_hybrid(df: pd.DataFrame, timeframe: str, params: dict) -> List[Dict]:
    ...
    for i, channel in enumerate(channels):
        ...
        if total_boundary_touches < params.get("min_boundary_touches", 4):  # tunable (R-2), default 4
            continue
        ...
        if pct_closes_in_channel < params.get("min_pct_closes_in_channel", 85.0):  # tunable (R-2), default 85%
            continue
