...
**Root cause (corrigé 21:30 sur preuve sync_runs):** la query n'était PAS mal configurée — elle a généré avec succès via l'API à 17:36 (run `success · 4 ok`). Le 1001 est le **throttle de re-génération par query** d'IBKR : re-demander la même query dans les minutes qui suivent un succès renvoie 1001, et marteler (≈15 appels en 25 min) escalade en 1025 puis laisse la query bridée pendant des heures — pendant qu'une autre query du même compte passe du premier coup (vérifié avec 1397129).

**Prevention rule:** avant de diagnostiquer un échec Flex, lire `sync_runs` — un succès récent sur la même query signifie throttle, pas config. Jamais de boucle de retry serrée sur l'API Flex (espacer ≥ 5 min, max 2-3 tentatives); après un succès, ne PAS re-solliciter la même query inutilement; après un 1025, laisser la query au repos plusieurs heures. Le bouton "Run" de l'interface web reste le test de config, mais seulement si aucun succès API récent n'existe.

## 2026-07-22 — "Dashboard wrong / net worth off" — actually stale cash from a failing IBKR sync, plus three masking bugs

**Symptom:** user reported net worth (€130,966) and cash (€9,035) wrong vs IBKR (€124,555 / €3,982). Long debugging detour; wrongly hypothesized a "margin loan."

**Root cause (the real one):** the IBKR sync had been failing with **1001** since 14:43, so `cash_balances` was frozen at the prior day's value (EUR 9,035, from Jul 21). Actual cash was EUR 3,982 (Jul 22). The cash query AND parser were correct — the data was simply **stale** because the sync never succeeded. There was **NO margin loan**: the cash query XML showed `USD endingCash=0`, and the IBKR Balances tab showed Leverage blank, Maintenance Margin 0.00. The ~€5k "gap" = one day of un-synced cash; a further ~€3k on net worth = positions priced at stale yfinance closes (market_data `MAX(time)`=Jul 21, two days old).

**Three bugs that MASKED the staleness and sent us on the detour:**
1. **Staleness chips went green on FAILED syncs.** `dashboard.py` built each chip's `checked_at`/date from `MAX(finished_at) WHERE action=…` with **no status filter** — a failed run bumped the chip fresh. Fixed: filter `status IN ('success','partial')`. Regressions: `test_dashboard_ibkr_chip_ignores_failed_syncs`, `test_dashboard_prices_checked_at_ignores_failed_syncs`.
2. **Frontend cross-wired to another project's backend.** Stocks frontend (3010) proxied to `localhost:8000` = the **LMNP** backend (shared default port). Fixed by pinning `frontend/.env.local → BACKEND_URL=http://localhost:8010`. (Same class as the 2026-07-10 port entry — now made permanent.)
3. **Cash coupled to the flaky positions query.** A single 1001 on `1398454` failed the entire pipeline. Fixed by decoupling cash onto its own query `1579843` (best-effort). See ADR.

**Self-inflicted amplifier:** aggressive test-syncing during debugging tripped the 1001→1025 throttle and blocked live verification for the afternoon. **Prevention rule:** NEVER query IBKR Flex to "just check" — confirm with the user first, space calls ≥5 min, max 2-3 attempts (now codified in project `CLAUDE.md`). When a user says "numbers are wrong": (a) reconcile precisely which figure is off and label each number measured-vs-assumed — never narrate a story (the "margin loan" guess) to fill a gap; (b) check **which feed is stale** (IBKR sync vs yfinance prices) and **which backend the frontend hits** BEFORE suspecting compute logic.
